...
function middle_configuration(model::CentroidalQuadrupedWall)
    x_shift = 0.0
    pitch_shift = 0.0 
    [
        -0.05; 0.0; body_height;
        0.0; pitch_shift; 0.0;
...
qM = middle_configuration(model)
qT = final_configuration(model)
q_ref = [sinusoidal_interpolation(q1, qM, Tm)...,sinusoidal_interpolation(qM, qT, Tm)...]
...
[q_ref[Tm+i][10] = FR_foot_x[i] for i in 1:Tm] 
[q_ref[Tm+i][12] = FR_foot_z[i] for i in 1:Tm] 
q_ref = [q1, q_ref...]
